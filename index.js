...
// إضافة هذه الدوال إلى كودك الحالي

// تحسينات للعرض على الجوال
function optimizeForMobile() {
    // تكبير حجم اللاعب والنفايات على الهواتف
    if (window.innerWidth <= 768) {
        player.width = 90;
        player.height = 90;
        player.speed = 8; // زيادة السرعة قليلاً
        ...
        // تكبير حجم صناديق القمامة
        bins.forEach(bin => {
            bin.width = 120;
            bin.height = 140;
        });
        
        // إخفاء رسالة الأوامر بعد وقت على الجوال
        setTimeout(() => {
...
function showMobileTips() {
    if (isMobileDevice()) {
        ...
        tip.innerHTML = `
            <h3>🎮 نصائح للعب</h3>
            <p>استخدم عصا التحكم للحركة</p>
            <p>اضغط زر الإسقاط لوضع النفايات</p>
            <button onclick="this.parentElement.parentElement.remove()" style="margin-top:10px; padding:8px 15px;">حسناً</button>
        `;
        document.getElementById('game-container').appendChild(tip);
        
        // إخفاء التلميح تلقائياً بعد 8 ثواني
        setTimeout(() => {
...
function optimizePerformance() {
    // تقليل عدد حبات الكونفيتي على الجوال
    if (isMobileDevice()) {
        window.createConfetti = function() {
            for (let i = 0; i < 30; i++) { // 30 بدلاً من 60
                const confetti = document.createElement('div');
                confetti.className = 'confetti';
                confetti.style.left = `${Math.random() * 100}%`;
                confetti.style.top = `${Math.random() * 100}%`;
...
function resizeCanvas() {
    ...
    if (gameActive) {
        player.x = canvas.width / 2;
        player.y = canvas.height - 200;
        optimizeForMobile(); // استدعاء التحسينات
        spawnBins();
        spawnTrash();
    }
}

// إضافة CSS ديناميكي للجوال
function addMobileCSS() {
    ...
    style.textContent = `
        @media (max-width: 768px) {
            ...
            /* إخفاء رسالة الأوامر بعد وقت */
            .command-message {
                font-size: 14px !important;
                top: 10px !important;
                opacity: 0 !important; /* إخفاء على الجوال */
            }
...
window.addEventListener('load', function() {
    ...
    // إذا كان جوال، أضف زر ملء الشاشة
    if (isMobileDevice()) {
...
// تحديث دالة setupEventListeners لإضافة دعم أفضل للجوال
function setupEventListeners() {
    startButton.addEventListener('click', () => {
        homeScreen.style.display = 'none';
        gameScreen.style.display = 'flex';
